<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <ul class="flex">
            @foreach($languages as $lang)
                <li class="py-3 px-6 border-r border-gray-300
                    ...
                    @endif"
                >
                    <a href="{{ route('businesses.edit', ['groupId' => $groupId, 'languageId' => $lang->id]) }}">
                        {{ $lang->label }}
                    </a>
                </li>
            @endforeach
    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
        <livewire:business.show :group-id="$groupId" :language-id="$languageId" wire:key="'{{$groupId . '-'. $lang->id}}'"/>
    </div>
</div>
